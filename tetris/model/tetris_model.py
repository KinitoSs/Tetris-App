...
class TetrisModel(QWidget):
    closed = pyqtSignal()  # signal attribute for parent window

    def __init__(self, play_window: IPlayWindow):
    ...
    def GameStateUpdate(self):
        ...
        if self.pause != 1:
            ...
            if checkLost(locked_positions):
                ...
                self.leadb = Leaderboard(
                    score, 6 - level
                )  # create leaderboard child window
                self.leadb.closed.connect(
                    self.show
                )  # show main title window when closing leaderboard window
                self.leadb.show()
        ...
        for y in range(20):
            for x in range(15):
                ...
                if self.tableWidget.item(y, x).text() == "0":
                    blockColor = QtGui.QColor(0, 0, 0)  # Black
                if self.tableWidget.item(y, x).text() == "1":
                    blockColor = QtGui.QColor(255, 0, 0)
                if self.tableWidget.item(y, x).text() == "2":
                    blockColor = QtGui.QColor(0, 255, 0)
                if self.tableWidget.item(y, x).text() == "3":
                    blockColor = QtGui.QColor(0, 0, 255)
                if self.tableWidget.item(y, x).text() == "4":
                    blockColor = QtGui.QColor(255, 255, 0)
                if self.tableWidget.item(y, x).text() == "5":
                    blockColor = QtGui.QColor(255, 0, 255)
                if self.tableWidget.item(y, x).text() == "6":
                    blockColor = QtGui.QColor(0, 255, 255)
                if self.tableWidget.item(y, x).text() == "7":
                    blockColor = QtGui.QColor(255, 255, 255)
